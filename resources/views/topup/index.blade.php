...
<div class="my-3 p-3 bg-body rounded shadow-sm">
    <!-- FORM PENCARIAN -->
    <div class="pb-3">
      <form class="d-flex" action="{{ url('topup')}}" method="get">
    ...
    <!-- TOMBOL TAMBAH DATA -->
    <div class="pb-3">
      <a href='{{ url('topup/create') }}' class="btn btn-primary">+ Tambah Data</a>
    </div>
    ...
    <table class="table table-striped">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $item->jurusan}}</td>
                <td>
                    <a href='{{ url('topup/'.$item->nim.'/edit') }}' class="btn 
                        btn-warning btn-sm">Edit</a>
                        <form onsubmit="return confirm('Yakin akan menghapus data?')"
                        class='d-inline' action="{{ url('topup/'.$item->nim) }}"
                        method="post">
                        @csrf
                        @method('DELETE')
